...
public class ExpressionAddOperators {
    public static void main(String[] args) {
        ...
        List<String> res = expressionAddOperators.expressionAddOperators(num, target);
    ...
    public void backTrack(String exp, List<String> list, int curr, String num, int target, long total, long prod) {
        if (curr == num.length()) {
            if (total == target) {
                list.add(exp);
            }
            ...
            for(int i = curr, l = num.length(); i < l; i++) {
                ...
                if (curr == 0) {
                    ...
                    backTrack(exp+"*"+newNum, list, i+1, num, target, total-prod+(prod*newNumL), newNumL*prod);
                }
            }
        }
